...
basic_robot.commands.insert_item = function(name,item, inventory,dir)  
	...
	if (item == "basic_machines:ball_spell" or
		item == "default:key" or
		item == "default:skeleton_key" or
		item == "telemosaic:key") or string.match(tools, item) then
	
	error("You can't move tools with robot!")		
	return
	end

	local long_string = string.len(item)	

	if long_string > 50 then
	error("Too long string name! Make it less.")
	return
	end
	...
	error("Your stack value is too huge, try to make it less.")   
	return false
	end	
