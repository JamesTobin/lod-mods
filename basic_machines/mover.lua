...
basic_machines.hardness["basic_machines:mover"]=9999999.;
basic_machines.hardness["basic_machines:keypad"]=0.;
basic_machines.hardness["basic_machines:distributor"]=0.;
basic_machines.hardness["basic_machines:battery_0"]=9999999.;
...
basic_machines.hardness["basic_machines:generator"]=9999999.; -- can only place generator by hand
...
basic_machines.hardness["locked_travelnet:travelnet"]=10^8;
basic_machines.hardness["locked_travelnet:elevator"]=10^8;
basic_machines.hardness["travelnet:travelnet"]=10^8;
basic_machines.hardness["travelnet:elevator"]=10^8;
...
basic_machines.limit_inventory_table = { -- node name = {list of bad inventories to take from} OR node name = true to ban all inventories
	...
	["pipeworks:autocrafter"]= {["recipe"]=1, ["output"]=1},
	...
	["smartshop:shop"] = true,
	["xdecor:workbench"] = true,
	["realchess:chessboard"] = true,
}
...
local function check_abuse(prefer)
      if prefer and prefer ~= "" then
		...
		local long_string = string.len(item)
		local filter = string.len(prefer)		

		local t = {}
		for name in pairs(minetest.registered_tools) do
			t[#t+1] = name
		end

		local tools = table.concat(t, "\n")
				

		local ct = string.match(prefer, "[%S+]%s[%S+]%s(.+)")

		if ct or string.match(tools, item) or filter > 50 then
			prefer = "Incorrect name"
		end
